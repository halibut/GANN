...
@Test
class XORCombinationTest {

	import Assert._
	
	val combineFunc = XORCombination
	
	@Test
	def testFalse_2Inputs_11(){
	...
	def testTrue_ManyInputs(){
		val c = combineFunc.combineInputs(Array(1,0,1,1,0))
	...
	def testFalse_ManyInputs(){
		val c = combineFunc.combineInputs(Array(1,0,1,1,0,1))
